...
const formatDetailWithIcon = (icon, value, label) => {
    ...
    return `
        <div class="flex">
            <img class="img-48 mr-sm" src="icons/car-details/${icon}.svg" alt="${label}">
            <div class="flex fd-col ml-sm">
                <span class="b2 bold upper">${value}</span>
                <span class="b2 italic light txt-dark">${label}</span>
            </div>
        </div>
    `;
};
...
function showHorsepowerInfo(event, engineTechnology, displacementCC, cv, kw, torque, cylinders) {
    const getContent = () => `
        ...
        <div class="px-lg pblk-md">
            <span class="b2 light">A continuación se presentan las especificaciones detalladas de potencia del vehículo:</span>
            
            <div id="carOverviewPopupContainer" class="popup-details pblk-md">
...
function showBodyTypeInfo(event, bodyType, weight, length, width, height, trackWidth, wheelbase) {
    ...
    const getContent = () => `
        ...
        <div class="px-lg pblk-md">
            <span class="b2 light">A continuación se presentan las especificaciones detalladas de la carrocería del vehículo, incluyendo dimensiones y peso:</span>
...
function showConsumptionInfo(event, brand, model, averageConsumption, urbanConsumption, highwayConsumption, fuelTankCapacity, rawAverageConsumption) {
    ...
    const contentTemplate = (message) => `<span class="b2 light">El <span class="medium">${brand} ${model}</span> ${message}</span><br><br>`;
    ...
    const getContent = () => `
        ...
        <div class="px-lg pblk-md">
            ...
            <div id="carOverviewPopupContainer" class="popup-details pblk-md">
                ...
                ${formatDetailWithIcon("range-tank", `${fuelTankCapacity} L`, "Capacidad del Depósito")}
            </div>
            <span class="b2 light">Con un depósito lleno, puedes esperar una autonomía de hasta <span class="medium">${Math.round(maxDistance)} km</span>.</span>
            ${getVariationContent()}
            ${getContactInfo()}
        `;
...
function showEmissionsInfo(event, emissionLabel, co2Emissions) {
    ...
    const createEmissionContent = (description, benefits, range) => `
        <div class="px-lg pblk-md">
            <span class="b2 light">${description}</span>
            <div id="carOverviewPopupContainer" class="popup-details pblk-md">
            ...
            <span class="b2 light"><strong>Los beneficios incluyen:</strong></span>
            <ul class="px-md b2 light">
...
function showDiscountInfo(event, price, previousPrice, savingsAmount) {
    const getContent = () => `
        ...
        <div class="px-lg pblk-md">
            <span class="b2 light">Aprovecha este increíble descuento en nuestro vehículo seleccionado:</span>
...
//////////////////////////////////////    NEW FILE popupFinance.js NEEDS TO BE CREATED   /////////////////////////////////////
...
export function showDocumentsInfo(event) {
    const getContent = () => `
        ...
        <div class="px-lg pblk-md b2">
            <div class="light">Para la financiación es necesario presentar los siguientes documentos:</div>
            <br>
            <div class="bold upper">Si eres particular:</div>
            <ul class="px-md mblk-sm pb-sm light">
                <li>DNI</li>
                <li>Última nómina</li>
                <li>Cuenta bancaria</li>
            </ul>
            <div class="bold upper">Si eres autónomo:</div>
            <ul class="px-md mblk-sm pb-sm light">
                <li>DNI</li>
                <li>Cuenta bancaria</li>
            ...
            <div class="bold upper">Si eres empresa:</div>
            <ul class="px-md mblk-sm pb-sm light">
                <li>Cuenta bancaria</li>
                <li>DNI apoderado</li>
                <li>IVA anual y trimestral</li>
