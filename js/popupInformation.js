...
function getContactInfo(phone = '982 21 74 03', address = 'Estrada de A Coruna, 50, 27003 Lugo') {
    return `
        ...
        <br>
        <strong class="b2">Teléfono:</strong> <span class="b2 font-book">${phone}</span><br>
        <strong class="b2">Dirección:</strong> <span class="b2 font-book">${address}</span><br>
    `;
}
...
function showHorsepowerInfo(event, engineTechnology, displacementCC, cv, kw, torque, cylinders) {
    const getContent = () => `
        ...
        <div class="p-lg">
            <span class="b2 font-book">A continuación se presentan las especificaciones detalladas de potencia del vehículo:</span>
            
            <div id="carOverviewPopupContainer" class="car-details pt-md">
...
function showBodyTypeInfo(event, bodyType, weight, length, width, height, trackWidth, wheelbase) {
    ...
    const getContent = () => `
        ...
        <div class="p-lg">
            <span class="b2 font-book">A continuación se presentan las especificaciones detalladas de la carrocería del vehículo, incluyendo dimensiones y peso:</span>
...
function showConsumptionInfo(event, brand, model, averageConsumption, urbanConsumption, highwayConsumption, fuelTankCapacity, rawAverageConsumption) {
    ...
    const contentTemplate = (message) => `<span class="b2 font-book">El <strong>${brand} ${model}</strong> ${message}</span><br><br>`;
    ...
    const getContent = () => `
        ...
        <div class="p-lg">
            ...
            <br>
            <span class="b2 font-book">Con un depósito lleno, puedes esperar una autonomía de hasta <strong>${Math.round(maxDistance)} km</strong>.</span>
            ${getVariationContent()}
            ${getContactInfo()}
        `;
...
function showEmissionsInfo(event, emissionLabel, co2Emissions) {
    ...
    const createEmissionContent = (description, benefits, range) => `
        <div class="p-lg">
            <span class="b2 font-book">${description}</span>
            <div id="carOverviewPopupContainer" class="car-details pt-md">
            ...
            <span class="b2 font-book"><strong>Los beneficios incluyen:</strong></span>
            <ul class="ulPopup">
...
function showDiscountInfo(event, price, previousPrice, savingsAmount) {
    const getContent = () => `
        ...
        <div class="p-lg">
            <span class="b2 font-book">Aprovecha este increíble descuento en nuestro vehículo seleccionado:</span>
