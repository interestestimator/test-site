 ...
 */
function getPriceEntryClasses(includeCheckbox = false) {
    return includeCheckbox ? `${priceEntryClasses} ${checkboxWrapperClass}` : priceEntryClasses;
}
...
function appendDiv(className, innerHTML, container) {
    ...
    div.style.minHeight = '2rem'; // Set minimum height
...
function appendItems(items, container) {
    items.forEach(({ description, condition }) => {
        if (condition) {
            appendDiv(
                getPriceEntryClasses(),
                `<span class="b2 light txt-darker">${description}</span>
...
function initializeCheckboxes(currentCarData) {
    ...
    if (currentCarData.isExchangeAvailable) {
        appendDiv(
            getPriceEntryClasses(),
            `<span id="exchangeAmount_title" class="b2 light txt-darker">Entrega vehículo a cambio</span>
            <a href="/vender-mi-vehiculo.html?id=${currentCarData.carId}" class="btn round-xlg px-lg pblk-sm bg-darkist">
