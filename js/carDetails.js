...
const createDetailDiv = (containerId, iconPath, name, value, description, additionalInfo = '') => {
    ...
    detailDiv.innerHTML = `
        <img src="${iconPath}" alt="${description}" width="48" height="48">
        <div class="detail-text b1">
            <span class="font-bold uppercase">${value}</span>
            <span class="txt-grey">${description}</span>
            ${additionalInfo ? `
                <label class="more-information ${name}">
...
const getIconPath = (name, iconPathExt, iconFolder, defaultIconFolder) =>
    iconFolder ? `icons/car-details/variables/${name}-icons/${iconPathExt}.svg` : `${defaultIconFolder}/${name}.svg`;
...
const initializeOverviewDetails = (currentCarData, isNewCar = false) => {
    ...
    const overviewDetails = [
        ...
        { name: 'year', description: 'Matriculación' },
        { name: 'bodyType', description: 'Carrocería', additionalInfo: true, iconFolder: true },
        { name: 'doors', description: 'Nº Puertas' },
        { name: 'seats', description: 'Nº Asientos' },
        { name: 'color', description: 'Color Exterior', iconFolder: true },
        { name: 'averageConsumption', description: 'Consumo de Combustible', additionalInfo: true },
        { name: 'emissionLabel', description: 'Emisiones CO2', value: `${currentCarData.co2Emissions} g/km`, iconFolder: true, additionalInfo: true }
    ].filter(({ name }) => !(isNewCar && (name === 'kilometres' || name === 'year')));
