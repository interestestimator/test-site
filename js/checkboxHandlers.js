...
/**
 * Toggles the 'font-fade' class on all elements with the 'entry__amount' class within the specified parent element.
 * @param {HTMLElement} parentElement - The parent element to search within.
 * @param {boolean} add - If true, adds the 'font-fade' class; if false, removes it.
 */
function toggleGreyClassOnEntryAmounts(parentElement, add) {
    ...
    // Loop through the selected elements and toggle the 'font-fade' class based on the 'add' parameter
    entryAmountElements.forEach(element => {
        if (add) {
            element.classList.add('font-fade'); // Add the class if 'add' is true
        } else {
            element.classList.remove('font-fade'); // Remove the class if 'add' is false
        }
    });
}
