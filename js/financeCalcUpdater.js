...
function generateLoanContent({ formattedPmt, totalLoanAmount, deposit, loanTermMonths, installmentAmount, initialFeePercent, annualRateTIN, annualRateTAE, includeDocumentsButton, includeAcknowledgeButton }) {
    return ` 
        <div class="px-lg pblk-md b2 ${includeDocumentsButton ? 'txt-medium' : ''}">  
        ...
        <div class="show-documents-button btn round-xlg mx-md px-lg pblk-sm bg-darker center">
            <span class="h4 txt-medium">Documentación necesaria</span>
        </div>` : ''}    
        <div class="pt-sm mx-lg b1 ${includeDocumentsButton ? 'txt-dark' : ''}">  
            <h4 class="upper">Condiciones de financiación</h4>
            <span class="light">Condiciones legales Adevinta</span>
            <p class="light italic b4">
            ...
            <div class="pt-sm" style="width: 33%;margin-left: auto;">
                <div id="acknowledgeButton" class="btn-op round-xlg p-sm bg-darkist center txt-light b1">Entendido</div>
            </div>` : ''}
        </div>
    `;
}
        ...
        if (resetButton) {
            resetButton.addEventListener('click', () => {
                ...
                if (resetIcon) {
                    ...
                    resetIcon.classList.remove('btn');
                }
            });
        } else {
...
function initializePmtContainer(pmtValue) {
    ...
    financePmtContainer.innerHTML = `
        <span id="pmtValue" class="px-lg b1">
            <span class="b2 light txt-darker">Financiado desde</span><br>
            <span class="h2">
                ${pmtValue}*<span class="b4">/mes</span>
            </span>
        </span>
        <a href="#vehicle-financing" class="btn-op rl-lg px-lg pblk-sm bg-darkist">
            <span class="center txt-light h4 italic">Recalcular</span>
        </a>
...
function generateFinanceMessage(carReference, financePrice) {
    ...
    return `
        <div>Por favor, completa el formulario para recibir más información sobre las opciones de financiación para el <strong>${carReference}</strong>. Ya hemos registrado tus preferencias:</div>
        <ul>
        ...
        <div>Si deseas añadir un comentario adicional, puedes hacerlo a continuación.</div>
    `;
}
