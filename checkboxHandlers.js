    ...
    if (event.target.checked) {
        ...
        try {
            ...
            if (deliveryRegions.length > 0) {
                ...
                updatePrices(state, state.currentCarData.rawPrice, state.undisclosedFinanceCost);
            }
 ...
 */
function handleFinanceCheckboxChange(event, state, rawPrice, rawfinancePrice) {
    state.financeCharge = event.target.checked ? 0 : state.undisclosedFinanceCost;
    updatePrices(state, rawPrice, rawfinancePrice);
 ...
 */
function updatePrices(state, rawPrice, rawfinancePrice) {
    ...
    console.log(newPriceLabel) //////// THIS IS LOGGED 4 TIMES FOR EACH CLICK
