...
/**
    ...
    function appendFreePackage(description, container) {
        appendDiv(
            'price-group__entry',
            `<span class="b2">${description}</span>
            <span class="entry__amount b1 bold upper">GRATIS</span>`,
            container
        );
    }
    ...
    function appendExtrasPackage(id, title, value, container) {
        appendDiv(
         ...
         <label for="${id}Checkbox" class="mr-sm"></label>
         <span class="b2" style="margin-right: auto;">${title}</span>
    ...
    function initializeCheckboxes(currentCarData) {
        ...
        if (financeDiscount > 0) {
            appendDiv(
             ...
             <label for="financeCheckbox" class="mr-sm"></label>
             <span id="financeAmount_title" class="b2" style="margin-right: auto;">Descuento por financiación</span>
             <span class="entry__amount b1 upper bold" id="financeAmount">-${formatCurrency(financeDiscount)}*</span>`,
        ...
        if (currentCarData.isExchangeAvailable) {
            appendDiv(
                'price-group__entry',
                `<span id="exchangeAmount_title" class="b2">Entrega vehículo a cambio</span>
        ...
        if (currentCarData.isDeliveryAvailable) {
            appendDiv(
             ...
             <label for="deliveryCheckbox" class="mr-sm"></label>
             <span id="deliveryLabel_title" class="b2" style="margin-right: auto;">Entrega a domicilio</span>
             <select id="regionDropdown" style="display: none; margin-right: auto;"></select>
