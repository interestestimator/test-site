...
function createInputField(type, name, placeholder, required = false, extraAttributes = '') {
    return `
        <div class="input-box pt-sm">
            <input class="round-sm px-md oh bg-light b2" type="${type}" id="${name}" placeholder="${placeholder}" ${required ? 'aria-required="true"' : ''} maxlength="30" autocapitalize="none" autocorrect="off" name="${name}" ${required ? 'required' : ''} ${extraAttributes}>
            <span class="px-md pt-md b3 light" for="${name}">${placeholder}</span>
        </div>
    `;
}
...
function createRadioField(name, label, options) {
    return `
        <div class="input-box pt-sm">
            <div class="pb-sm italic light txt-dark">${label}</div>
            <div class="form-options flex round-lg oh">
                ${options.map(option => `
                    ...
                    <label for="${option.id}" class="button-like pblk-sm center bg-light btn">${option.label}</label>
                `).join('')}
            </div>
        </div>
    `;
...
function getFormFields(formType, carId, financeDetails) {
    ...
    const additionalFields = {
        ...
        [FORM_TYPES.REQUEST_FINANCE]: `
            ...
            <div class="input-box pt-sm">
                <label id="finance_char_count" for="finance_comments" class="italic light txt-dark">Comentarios sobre la financiación (máx. 350 caracteres)</label>
                <textarea id="finance_comments" name="finance_comments" class="mblk-xsm round-sm px-md pt-sm oh bg-light b1" placeholder="Tus comentarios" rows="4" maxlength="350"></textarea>
            </div>
        `
    };
...
function showContactForm(event, formType, carId = '', carReference = '', financePrice = '', cashPrice = '', financeDetails = '') {
    ...
    const getContent = () => `
        ...
        <section class="px-lg pblk-md b2">
            <div class="pb-sm light">${formDescription}</div>
            <form id="${formType}Form">
                ...
                <span class="button-box">
                    <button class="form-submit-button round-xlg px-lg pblk-sm bg-accent center h4 txt-light" type="submit" name="submit" disabled>
