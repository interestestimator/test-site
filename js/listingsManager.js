...
const generateOptionsHtml = (containerId, options, iconFolder, carsData = []) => {
    ...
    const optionsHtml = options.map(option => {
        ...
        return `
            <p class="popup-option flex space-between" data-value="${option.value}">
                <img class="img-24 mr-sm" src="${iconFolder}/${option.value}.svg" alt="${option.value} icon"/>
                <span class="font-book" style="margin-right: auto;">${displayValue}</span>
    ...
    return `
        <div class="option-group">
            ${optionTitle ? `<h4 class="option-group-title b1 uppercase">${optionTitle}</h4>` : ''}
            ${optionsHtml}
        </div>`;
};
