...
function setInitialValues(vehiclePrice, elements) {
    ...
    elements.financeCalcDepositSlider.max = maxDeposit || elements.financeCalcDepositSlider.min;
    elements.financeCalcDepositSlider.value = maxDeposit / 2;
    elements.financeCalcDepositInput.value = formatCurrency(elements.financeCalcDepositSlider.value);
    elements.financeCalcMaxDepositText.textContent = `Máxima: ${formatCurrency(maxDeposit)}`;

    // Set initial value for term slider and select
    elements.financeCalcTermSlider.value = financeTerm || elements.financeCalcTermSlider.min;
    elements.financeCalcTermSelect.value = elements.financeCalcTermSlider.value;
}

function updateLoanInfo(vehiclePrice, annualRateTIN, commissionRate, elements) {
    const vehicleDeposit = parseInt(elements.financeCalcDepositInput.value.replace(/[^\d]/g, ''));
    const termValue = parseInt(elements.financeCalcTermSelect.value);
...
function initializeFinanceCalcEventListeners(vehiclePrice, annualRateTIN, commissionRate, elements) {
    ...
    elements.financeCalcDepositSlider.addEventListener('input', function () {
        elements.financeCalcDepositInput.value = formatCurrency(this.value);
        updateLoanInfo(vehiclePrice, annualRateTIN, commissionRate, elements); // Update details when slider changes
    });

    elements.financeCalcDepositInput.addEventListener('input', function () {
        const numericValue = parseInt(this.value.replace(/[^\d]/g, ''));
        if (!isNaN(numericValue) && numericValue <= elements.financeCalcDepositSlider.max) {
            elements.financeCalcDepositSlider.value = numericValue;
            updateLoanInfo(vehiclePrice, annualRateTIN, commissionRate, elements); // Update details when input value changes
        }
    });

    elements.financeCalcTermSlider.addEventListener('input', function () {
        elements.financeCalcTermSelect.value = this.value;
        updateLoanInfo(vehiclePrice, annualRateTIN, commissionRate, elements); // Update details when slider changes
    });

    elements.financeCalcTermSelect.addEventListener('change', function () {
        elements.financeCalcTermSlider.value = this.value;
