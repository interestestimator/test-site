...
const PHOTO_COUNT_ICON_PATH = "icons/gallery-icons/camera.svg";
...
function updateImage(state, carImage, photoCountLabel, thumbnailContainer, galleryType) {
    const { currentImageIdx, imagesUrlList, colors } = state.galleries[galleryType];
    ...
    const currentColor = colors?.[currentImageIdx] || "";
    const imageColour = currentColor ? `: ${currentColor}` : "";

    photoCountLabel.innerHTML = `
        <img src="${PHOTO_COUNT_ICON_PATH}" alt="Info Icon">
        ${currentImageIdx + 1} of ${imagesUrlList.length}${imageColour}
    `;

    const thumbnailLimit = state.thumbnailLimit;

    // Update `thumbnailStartIdx` only if current image index is out of bounds
    if (currentImageIdx < state.thumbnailStartIdx) {
        // Moving left and out of the current thumbnail range
        state.thumbnailStartIdx = Math.max(currentImageIdx, 0);
    } else if (currentImageIdx >= state.thumbnailStartIdx + thumbnailLimit) {
        // Moving right and out of the current thumbnail range
        state.thumbnailStartIdx = Math.min(currentImageIdx - thumbnailLimit + 1, imagesUrlList.length - thumbnailLimit);
    }
