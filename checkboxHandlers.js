    ...
    if (event.target.checked) {
        ...
        try {
            ...
            if (deliveryRegions.length > 0) {
                ...
                updatePrices(state, elements, state.currentCarData.rawPrice, state.undisclosedFinanceCost);
            }
 ...
 */
function handleFinanceCheckboxChange(event, state, elements, rawPrice, rawfinancePrice) {
    state.financeCharge = event.target.checked ? 0 : state.undisclosedFinanceCost;
    updatePrices(state, elements, rawPrice, rawfinancePrice);
}
 ...
 */
function updatePrices(state, elements, rawPrice, rawfinancePrice) {
