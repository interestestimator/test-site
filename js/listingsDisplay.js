...
function createPriceDetailsHTML(carData, financeData, index) {
    ...
    return `
        <div class="bg-light p-xsm">
            ...
            <div class="flex jc-between pb-xsm mx-xsm h3 bold">
...
function renderCarsList(carList, carBasePath, isNewCar, container) {
    carList.forEach((carData, index) => {
        ...
        const html = `
            <a href="${isNewCar ? `detalles-vehiculos-nuevos.html?id=${carData.carId}` : `detalles-vehiculo-ocasion.html?id=${carData.carId}`}" class="car-container round-lg mblk-xsm" id="car-${carData.brand}-${index}">
