...
/**
 * Calculates the rounded deposit value based on the total price.
 * @param {number} rawFinancePrice - The raw finance price of the vehicle.
 * @param {number} depositPercentage - The percentage used to calculate the deposit (default is 0.25).
 * @returns {number} - The rounded deposit value.
 */
function calculateRoundedDeposit(rawFinancePrice, depositPercentage = 0.25) {
    // Calculate the initial deposit
    const deposit = rawFinancePrice * depositPercentage;

    // Determine the rounding factor based on the vehicle price
    const roundingFactor = rawFinancePrice >= 10000 ? 500 : 100;

    // Calculate and return the rounded deposit
    return Math.round(deposit / roundingFactor) * roundingFactor;
}
