...
/*************************************************************************************/
/*********************** CAR HISTORY AND CONDITION DETAILS SECTION ******************/
/*********************** SECTION NEEDS TO BE IMPLEMENTED AND REFINED ****************/
/*************************************************************************************/

import { showWarrantyDetails, showManufacturerWarranty } from './popupHistory.js';

/**
 * Initializes car history and condition details section with data.
 * @param {Object} currentCarData - Object containing car data.
 */
const initializeHistoryDetails = (currentCarData) => {
    const defaultIconFolder = 'icons/history-details';

    // Define history and condition details array
    const historyDetails = [
        { name: 'owners', description: 'Nº Propietarios' },
        { name: 'warranty', description: 'Garantía General', additionalInfo: true },

        
        { name: 'serviceHistory', description: 'Historial de Mantenimiento', additionalInfo: true },
        { name: 'ITVStatus', description: 'Estado de ITV'},
        { name: 'inspectionDate', description: 'Fecha de Última Inspección ITV', additionalInfo: true },
        { name: 'accidentHistory', description: 'Historial de Accidentes', additionalInfo: true },
        { name: 'batteryWarranty', description: 'Garantía de Batería', additionalInfo: true },
        { name: 'engineWarranty', description: 'Garantía de Motor', additionalInfo: true },
        { name: 'recallStatus', description: 'Estado de Recalls', additionalInfo: true }
    ];

    // Create detail divs for each history detail
    historyDetails.forEach(({ name, description, value = currentCarData[name], iconFolder, additionalInfo }) => {
        if (!value) return; // Skip if no value is available
        ...
        createDetailDiv('carHistoryContainer', iconPath, name, value, description, infoText);
    });

    // Event handlers for additional information
    const historyHandlers = {
        warranty: (event) => showWarrantyDetails(event, currentCarData.rawWarranty),
        ITVStatus: (event) => showITVInfo(event, currentCarData.ITVStatus, currentCarData.inspectionDate),
        accidentHistory: (event) => showAccidentInfo(event, currentCarData.accidentHistory),
        batteryWarranty: (event) => showManufacturerWarranty(event, currentCarData.batteryWarranty, 'batería'),
        engineWarranty: (event) => showManufacturerWarranty(event, currentCarData.engineWarranty, 'motor')
    };

    // Add event listeners for dynamically created elements
    document.querySelectorAll('.more-information').forEach(el => {
        const handler = historyHandlers[el.classList[1]]; // Get the corresponding handler by class name
        if (handler) el.addEventListener('click', handler);
    });
};
