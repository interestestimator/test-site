...
import { calculateRate, getFinanceDetails } from './financeCalc.js';
...
function initializeFinanceCalcEventListeners(rawFinancePrice, rawAnnualRateTIN, initialFeePercent, deposit, loanTermMonths, elements) {
    const { depositSlider, depositInputField, termSlider, termSelectDropdown, resetValuesButton } = elements;
    const maxDeposit = rawFinancePrice * 0.6;
    ...
    const updateLoanDetails = () => updateLoanInfo(rawFinancePrice, rawAnnualRateTIN, initialFeePercent, elements);

    // Common handler for deposit input and slider
    const handleDepositChange = (numericValue) => {
        if (numericValue > maxDeposit) {
            alert(`El depósito no puede exceder ${formatCurrency(maxDeposit)}.`);
            return maxDeposit; // Adjust to max if exceeded
        }
    ...
    depositSlider.addEventListener('input', handleSliderChange);
    depositInputField.addEventListener('input', () => {
        const numericValue = handleDepositChange(parseDepositValue(depositInputField.value) || 0);
