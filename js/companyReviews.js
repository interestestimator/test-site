...
const generateRatingStars = (rating, isDecimal = false) => {
    return Array.from({ length: 5 }, (_, i) => {
        ...
        return `
            <svg class="img-24" xmlns='http://www.w3.org/2000/svg' viewBox='0 0 23.44 19'>
...
function loadReviews() {
    ...
    reviewsToDisplay.forEach(review => {
        ...
        reviewDiv.className = "review-item bg-darkist px-lg";
        reviewDiv.innerHTML = `
            <div class="rating flex pblk-md">${generateRatingStars(review.rating, false)}</div>
            <div class="b2 bold upper pb-xsm">${review.name}</div>
            <div class="b3 pb-xsm">${review.date}</div>
            <div class="b3 light italic txt-dark">${review.text}</div>
        `;
