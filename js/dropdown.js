...
function setupDropdowns(dropdownContainer) {
    const toggleDropdown = (title) => {
        ...
        dropdownContainer.querySelectorAll('.dropdown-content.show').forEach(content => {
            if (content !== targetElement) {
                ...
                otherTitle.classList.replace('bg-black', 'bg-dark');
    ...
    if (firstDropdown) {
        ...
        firstTitle.classList.replace('bg-dark', 'bg-black'); // Apply bg-black here
    }
}
    ...
    container.innerHTML = `
        ...
        ${sectionKeys.map((category, index) => `
            <div class="dropdown pb-sm b2">
                <label class="dropdown-title btn flex rr-lg jc-between italic bg-dark ai-center" data-target="content-${dataKey}-${index}">
                <span class="ml-md h3 txt-light">
                    ${category}
                    </span>
