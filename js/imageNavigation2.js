...
function renderThumbnails(state, thumbnailContainer, galleryType) {
    ...
    const thumbnails = state.galleries[galleryType].imagesUrlList.slice(state.thumbnailStartIdx, state.thumbnailStartIdx + state.thumbnailLimit).map((url, i) => {
        const thumbnail = document.createElement('img');
        thumbnail.src = url;
        thumbnail.alt = `Thumbnail ${state.thumbnailStartIdx + i + 1}`;
        thumbnail.className = `thumbnail${state.thumbnailStartIdx + i === state.galleries[galleryType].currentImageIdx ? ' active' : ''}`;
        return thumbnail;
    });
...
function updateImage(state, galleryType, carImage, photoCountLabel, thumbnailContainer) {
    ...
    carImage.src = state.galleries[galleryType].imagesUrlList[state.galleries[galleryType].currentImageIdx];
    carImage.alt = `Image ${state.galleries[galleryType].currentImageIdx + 1}`;
    photoCountLabel.innerHTML = `<img src="icons/ui/actions/photo-count.svg" alt="Info Icon" width="16" height="16">${state.galleries[galleryType].currentImageIdx + 1} of ${state.galleries[galleryType].imagesUrlList.length}`;

    // Update thumbnail index
    const thumbnailOutOfRange = state.galleries[galleryType].currentImageIdx < state.thumbnailStartIdx || state.galleries[galleryType].currentImageIdx >= state.thumbnailStartIdx + state.thumbnailLimit;
    if (thumbnailOutOfRange) {
        state.thumbnailStartIdx = Math.max(state.galleries[galleryType].currentImageIdx - state.thumbnailLimit + 1, 0);
    }
